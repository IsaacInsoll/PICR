...
Choose the layout that most closely matches your storage. This selection is synchronized with the installation guide.

<Tabs syncKey="picr-storage-layout">
  <TabItem label="Local storage">
    **Start with:** `FILE_WATCHER=native`

    Native events are efficient and immediate when PICR sees the same local filesystem that receives changes.

  </TabItem>
  <TabItem label="Docker or mounted share">
    **Start with:** `FILE_WATCHER=polling`

    Polling is dependable when Docker, SMB, NFS, or virtualisation does not deliver native events reliably.

  </TabItem>
  <TabItem label="Separate NAS">
    **Start with:** [PICR Ping](/PICR/operations/picr-ping/) and a scheduled safety scan.

    Ping watches beside the media while PICR verifies every hinted path through its own read-only mount.

  </TabItem>
  <TabItem label="Low-I/O or manual">
    **Start with:** `FILE_WATCHER=off`

    Add on-view, scheduled, or manual scans according to how quickly changes need to appear and whether NAS disks should sleep.
