...
1. Create persistent cache and database directories on the Docker host.
2. Add the PICR and PostgreSQL services to a Compose file.
3. Start the containers, retrieve the first password, and secure the administrator account.
4. Confirm the media library is visible, then publish PICR through HTTPS.
...
The example uses polling because it works reliably with many Docker and NAS mounts. Choose the tab that most closely matches your storage; the same choice remains selected in the detailed scanning guide.
...
<Tabs syncKey="picr-storage-layout">
  ...
  <TabItem label="Separate NAS">
    Use [PICR Ping](/PICR/operations/picr-ping/) for real-time hints and keep a
    scheduled scan as a safety net.
  </TabItem>
