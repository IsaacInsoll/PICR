...
1. Add the shoot to PICR's mounted media library and let it scan.
2. Give the folder a client-facing title, gallery image, and branding.
3. Create a scoped recipient link with the right proofing permissions.
4. Test the exact link in a private browser on desktop and mobile.
5. Send it, then monitor opens and review activity.
