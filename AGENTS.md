...
`app/src/graphql.schema.json` is tracked and the root `npm run gql` script
formats it, but it is not currently an output in `codegen.ts`. Do not assume it
was refreshed from the live backend schema just because `npm run gql` passed.
