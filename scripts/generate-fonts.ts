...
const buildAppFonts = (fontRegistry: Array<any>) => {
  ...
  fontRegistry.forEach((font) => {
    ...
    font.weights.forEach((weight: number) => {
      ...
      lines.push(`const ${varName} = require('./assets/fonts/${filename}');`);
  ...
  [400, 500, 700].forEach((weight) => {
    ...
    lines.push(
      `const roboto_${weight} = require('./assets/fonts/${filename}');`,
    );
...
const loadFontRegistry = async () => {
  ...
  const fontRegistry = (module.exports as { fontRegistry?: unknown })
    .fontRegistry;
