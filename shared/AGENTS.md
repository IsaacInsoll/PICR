...
`shared/urql/urqlClient.tsx` also imports GraphQL runtime values. Keep `graphql`
declared as both a peer dependency (the consumer provides the runtime instance)
and a dev dependency (standalone shared lint/typecheck/install). Every runtime
consumer that can reach that client, including `app`, must declare `graphql`
directly; do not rely on npm auto-installing it for
`@graphql-typed-document-node/core` under `shared/node_modules`.
...
1. Imports and validates the executable backend schema in a short-lived process
2. Writes an ignored introspection snapshot under `.scratch/`
3. Generates TypeScript types and typed document nodes for all operations
4. Updates `graphql.schema.json` for cache validation

The server and database do not need to be running. Always use the root
`npm run gql` wrapper so codegen cannot consume a stale temporary snapshot.
